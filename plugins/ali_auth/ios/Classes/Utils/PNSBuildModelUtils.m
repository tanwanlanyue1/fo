...
+ (TXCustomModel *)buildModelOption:(NSDictionary *)dict
    ...
    if (key && key.length > 0 && dict[key] != nil) {
      ...
      @try {
          ...
          if ([dict[key] isKindOfClass:[NSArray class]]) {
            ...
            [model setValue: mutableArray forKey:newKey];
          } else {
              /// 处理图片路径
              if ([key containsString:@"Path"]) {
                  /// 为路径时需要转换
                  ...
                  } else if ([key isEqual:@"lightColor"]) {
  ...
  if (model.privacyAlertIsNeedShow) {
    ...
    model.privacyAlertButtonFrameBlock = ^CGRect(CGSize screenSize, CGSize superViewSize, CGRect frame) {
      CGFloat btnWidth = [dict intValueForKey: @"privacyAlertBtnWidth" defaultValue: superViewSize.width / 2];
      CGFloat btnHeight = [dict intValueForKey: @"privacyAlertBtnHeigth" defaultValue: 40];
      CGFloat btnX = (superViewSize.width - btnWidth) / 2;
      CGFloat btnY = (superViewSize.height - btnHeight) - 16;
